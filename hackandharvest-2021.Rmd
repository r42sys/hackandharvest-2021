...
```
**Abbildung: Verteilung der Gattungen der Bäume der Stadt Konstanz für Gattungen mit mehr als `r N` Bäumen.**
...
Das Baumkataster der Stadt Konstanzer enthält `r length(unique(kataster[!is.na(kataster$name_de),'name_de']))` verschiedene Arten - `r nrow(kataster[is.na(kataster$name_de),])` Einträge haben hier keinen gültigen Wert.
...
ggplot(data) +
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
**Abbildung: Verteilung der Arten der Bäume der Stadt Konstanz für Arten mit mehr als `r N` Bäumen.**
...
Das Baumkataster der Stadt Konstanzer enthält `r nrow(kataster[is.na(kataster$hoehe),])` Einträge ohnen einen gültigen Wert für die Höhe.
...
Das Baumkataster der Stadt Konstanzer enthält `r nrow(kataster[is.na(kataster$kronendurchmesser),])` Einträge ohnen einen gültigen Wert für den Kronendurchmesser.
