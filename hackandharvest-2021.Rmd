...
kataster %>%
    ...
    ggplot() +
        geom_bar(aes(x=reorder(gattung, -table(gattung)[gattung]))) +
        labs(title='Verteilung der Gattungen', x='Gattung', y='Anzahl Bäume') +
        theme_bw() +
...
```
Anzahl der unterschiedlichen Gattungen
```{r}
length(unique(kataster[!is.na(kataster$gattung),'gattung']))
```

Anzahl der Bäume ohne einen gültigen Wert für die Gattung
```{r}
nrow(kataster[is.na(kataster$gattung),])
```
...
kataster %>%
    ...
    scroll_box(height = "300px")
```
Anzahl der unterschiedlichen Arten
```{r}
length(unique(kataster[!is.na(kataster$name_de),'name_de']))
```

Anzahl der Bäume ohne einen gültigen Wert für die Art
```{r}
nrow(kataster[is.na(kataster$name_de),])
```
...
ggplot(kataster %>% filter(!is.na(stammumfang))) +
    ...
    labs(title='Verteilung des Stammmumfang', x='Stammumfang [cm]', y='Anzahl Bäume') +
    theme_bw()
```
```{r}
kataster %>%
    ...
    scroll_box(height = "300px")
```
Anzahl der Bäume ohne einen gültigen Wert für den Stammumfang
```{r}
nrow(kataster[is.na(kataster$stammumfang),])
```
...
ggmap(map) +
    ...
    labs(title='Bäume in Konstanz (Stammumfang)') +
    theme_void() +
...
ggplot(kataster %>% filter(!is.na(hoehe))) +
    ...
    labs(title='Verteilung der Baumhöhe', x='Baumhöhe [m]', y='Anzahl Bäume') +
    theme_bw()
```
```{r}
kataster %>%
    ...
    scroll_box(height = "300px")
```
Anzahl der Bäume ohne einen gültigen Wert für die Höhe
```{r}
nrow(kataster[is.na(kataster$hoehe),])
```
...
ggmap(map) +
    ...
    labs(title='Bäume in Konstanz (Höhe)') +
    theme_void() +
    theme(legend.position="bottom") +
...
ggplot(kataster %>% filter(!is.na(kronendurchmesser))) +
    ...
    labs(title='Verteilung der Kronendurchmesser', x='Kronendurchmesser [m]', y='Anzahl Bäume') +
    theme_bw()
```
```{r}
kataster %>%
    ...
    scroll_box(height = "300px")
```
Anzahl der Bäume ohne einen gültigen Wert für den Kronendurchmesser
```{r}
nrow(kataster[is.na(kataster$kronendurchmesser),])
```
...
ggmap(map) +
    ...
    labs(title='Bäume in Konstanz (Kronendurchmesser)') +
    theme_void() +
    theme(legend.position="bottom") +
    scale_color_continuous(name='Kronendurchmesser')
```
...
Anzahl der Bäume ohne einen gültigen Wert für den Standort
```{r}
nrow(kataster[is.na(kataster$standort),])
```
...
ggmap(map) +
    geom_point(data=kataster %>% filter(!is.na(y) ), aes(x=x, y=y), color='green', size=0.25) +
    labs(title='Bäume in Konstanz (Standorte)') +
    theme_void() +
    theme(legend.position="bottom") 
```
...
O<sub>2</sub> Produktion in Tonnen pro Jahr für Konstanz
```{r}
sum(kataster$o2_produktion)/1000
```
...
ggmap(map) +
    ...
    labs(title=expression('Bäume in Konstanz ('*O[2]*' Produktion)')) +
    theme_void() +
    theme(legend.position="bottom") +
    scale_color_continuous(name=expression(Kilogramm*' '*O[2]*'  pro Jahr'), low = "yellow", high = "green")
```
...
```{r}
# CO2 Aufnnahme in Kilogram pro Jahr.
dfs = list(
    # Novak (2007a)
    kataster %>% 
        filter(stammumfang/pi <= to_centimeter(3)) %>%
...
CO<sub>2</sub> Aufnahme in Tonnen pro Jahr für Konstanz
```{r}
sum(kataster$co2_aufnahme)/1000
```
...
ggmap(map) +
    ...
    theme_void() +
    theme(legend.position="bottom") +
    scale_color_continuous(name=expression(Kilogramm*' '*CO[2]*'  pro Jahr'), low = "yellow", high = "green")
```

## Zusammenfassung

Ermöglicht wurde das ganze im Rahmen HACK AND HARVEST Hackathon 2021 mit:
1. Offene Daten ([Baumkataster der Stadt Konstanz](https://offenedaten-konstanz.de/comment/2445))
1. Freier Zugriff auf Forschungsergebnisse (siehe Literatur)
...
Nowak, David J.; Crane, Daniel E.; Oxygen production by urban trees in the United States; 2007b; Arboriculture & Urban Forestry 33(3); Heruntergeladen am 16.04.2021 von [hier](https://www.researchgate.net/publication/228633757_Oxygen_production_by_urban_trees_in_the_United_States)
